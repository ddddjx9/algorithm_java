...
public class MergeOrderedArray {
    public static void main(String[] args) {
        ...
        new MergeOrderedArray().merge(a1, 0, 2, 3, 6, a2, 0);
        System.out.println(Arrays.toString(a2));
        //如果想要将a1的数据更新为合并后的结果，只需要将a2中的元素拷贝给a1就可以了
    }

    /**
     * LeetCode88：将两个有序区间的的有序元素合并，合并后，结果仍然存储在原有空间
     *
     ...
     */
    public void merge(int[] nums1, int i, int iEnd, int j, int jEnd, int[] nums2, int k) {
        /*
        ...
        if (nums1[i] < nums1[j]) {
            nums2[k] = nums1[i];
            merge(nums1, i + 1, iEnd, j, jEnd, nums2, k + 1);
        } else {
            nums2[k] = nums1[j];
            merge(nums1, i, iEnd, j + 1, jEnd, nums2, k + 1);
        }
    }
}